...
export default class ORMAdapterPostgreSQL extends ORMAdapter {
    ...
    static formatCriteria(criteria) {
        ...
        return criteria.map((x, i) => `${(i === 0) ? '' : this.op(x[0] || '')} ${x[1] ? this.quoteColumn(x[1]) : ''} ${this.op(x[2] || '')} ${this.op(x[3] || '', true)} `).join("");
    }
    ...
    async add(models, weight, jointTableName, lk, fk) {
        ...
        return this.constructor.run(this.database, `INSERT INTO ${jointTableName} (${lk}, ${fk}, weight) VALUES ${values.join(', ')} ON CONFLICT DO NOTHING`, ids);
    }
    ...
    async readAll(kv, columns = ['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
        ...
        if (kv) {
            ...
            return this.readResult(limit, columns, `WHERE ${whereClause}${statementOrderBy.statement} LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, [...Array.from(kv.values()), ...statementOrderBy.values, limit, offset]);
        }
        return this.readResult(limit, columns, `${statementOrderBy.statement} LIMIT $1 OFFSET $2`, [...statementOrderBy.values, limit, offset]);
    }
    async readBy(key, values, columns = ['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
        ...
        return this.readResult(limit, columns, `WHERE ${this.constructor.quoteColumn(key)} IN (${placeholders})${statementOrderBy.statement} LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, [...values, ...statementOrderBy.values, limit, offset]);
    }
    async readWith(criteria, columns = ['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
        ...
        return this.readResult(limit, columns, `WHERE ${wheres} ${statementOrderBy.statement} LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, [...whereValues, ...statementOrderBy.values, limit, offset]);
    }
    ...
    async countBy(key, values) {
        if (!values || values.length === 0)
            return 0;
        const placeholders = this.constructor.generatePlaceholders(values.length);
        const result = await this.constructor.getRow(this.database, `SELECT COUNT(id) as count FROM ${this.tableName} WHERE ${this.constructor.quoteColumn(key)} IN (${placeholders})`, values);
        return Number(result?.count) || 0;
    }
    async countWith(criteria) {
        const { wheres, whereValues } = this.constructor.getWheresAndWhereValueFromCriteria(criteria);
        const result = await this.constructor.getRow(this.database, `SELECT COUNT(id) as count FROM ${this.tableName} WHERE ${wheres}`, whereValues);
        return Number(result?.count) || 0;
    }
    async deleteAll(kv = null) {
        ...
        const keys = Array.from(kv.keys());
        const whereClause = keys.map((k, i) => `${this.constructor.quoteColumn(k)} = $${i + 1}`).join(' AND ');
        return this.constructor.run(this.database, `DELETE FROM ${this.tableName} WHERE ${whereClause}`, Array.from(kv.values()));
    }
    async deleteBy(key, values) {
        if (!values || values.length === 0)
            return;
        const placeholders = this.constructor.generatePlaceholders(values.length);
        return this.constructor.run(this.database, `DELETE FROM ${this.tableName} WHERE ${this.constructor.quoteColumn(key)} IN (${placeholders})`, values);
    }
    ...
    async updateAll(kv, columnValues) {
        ...
        return this.constructor.run(this.database, `UPDATE ${this.tableName} SET ${setClause} WHERE ${whereClause}`, [...newValues, ...Array.from(kv.values())]);
    }
    async updateBy(key, values, columnValues) {
        ...
        return this.constructor.run(this.database, `UPDATE ${this.tableName} SET ${setClause} WHERE ${this.constructor.quoteColumn(key)} IN (${placeholders})`, [...newValues, ...values]);
    }
    async updateWith(criteria, columnValues) {
        ...
        return this.constructor.run(this.database, `UPDATE ${this.tableName} SET ${setClause} WHERE ${offsetWheres}`, [...Array.from(columnValues.values()), ...whereValues]);
    }
    async insertAll(columns, valueGroups, ids = []) {
        ...
        return this.constructor.run(this.database, `INSERT INTO ${this.tableName} (${this.constructor.quoteColumns(columns).join(', ')}) VALUES ${strValues.join(', ')}`, valueGroups.flat());
    }
}
