...
export default class ORMAdapterSQLite extends ORMAdapter {
  ...
  static op(operator: string | Function | any[], placeHolder = false): string | number {
    if (operator === '') return '';
    ...
    if (OP === undefined) {
      return (typeof operator === 'string') ? `'${operator}'` : operator as number;
    }
  ...
  async readAll(kv: Map<string, any>, columns=['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
    // @ts-ignore
  ...
  async countAll(kv: Map<string, any> = null) {
  ...
  async deleteAll(kv: Map<string, any> = null) {
    // @ts-ignore
  ...
  async updateAll(kv: Map<string, any>, columnValues: Map<string, any>) {
