...
export default class ORMAdapterSQLite extends ORMAdapter {
  static OP: Record<string, string | number> = Object.assign(
    { EQUAL: '=',
      GREATER_THAN: '>',
      LESS_THAN: '<',
      GREATER_THAN_EQUAL: '>=',
  ...
  static op(operator: string | Function | any[], placeHolder: boolean = false): string | number {
    if (operator === '') return '';
    if (typeof operator === 'function') return operator();
    if (Array.isArray(operator)) return operator[0];

    if (placeHolder) return '?';
    const OP = ORMAdapterSQLite.OP[operator as string];
    if (OP === undefined) {
  ...
  static async getRow(database: Database, sql: string, values: any[]) {
    try{
      // @ts-ignore
  ...
  static async getRows(database: Database, sql: string, values: any[]){
    try{
      // @ts-ignore
  ...
  static async run(database: Database, sql: string, values: any[]){
    try {
      // @ts-ignore
  ...
  async read(columns = ['id','name']) {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).getRow(this.database, sql, [this.client.id]);
  }

  async update(values: any[]) {
    const columns = this.client.getColumns();
    const sql = `UPDATE ${this.tableName} SET ${columns.map((x: string) => x + ' = ?').join(', ')} WHERE id = ?`;
    try{
      // @ts-ignore
      return (this.constructor as typeof ORMAdapterSQLite).run(this.database, sql, [...values, this.client.id]);
    }catch(e){
      Central.log(e);
      Central.log(sql);
      Central.log(values);
  ...
  async delete() {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${this.tableName} WHERE id = ?`, [this.client.id]);
  }

  async hasMany(tableName: string, key: string) {
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).getRows(this.database, `SELECT * FROM ${tableName} WHERE ${key} = ?`, [this.client.id]);
  }
  ...
  async belongsToMany(modelTableName: string, jointTableName: string, lk: string, fk: string) {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).getRows(this.database, sql, [this.client.id]);
  }
  ...
  async add(models: any[], weight: number, jointTableName: string, lk: string, fk: string) {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `INSERT OR IGNORE INTO ${jointTableName} (${lk}, ${fk}, weight) VALUES ${values.join(', ')}`, ids);
  }
  ...
  async remove(models: any[], jointTableName: string, lk: string, fk: string) {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, sql, ids);
  }

  async removeAll(jointTableName: string, lk: string) {
    Central.log('Removing all from ' + jointTableName + ' lk: ' + lk + ' id: ' + this.client.id);
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${jointTableName} WHERE ${lk} = ?`, [this.client.id]);
  }
  ...
  async readResult(limit: number, columns: string[], where: string, values: any[]) {
    ...
    if(limit === 1){
      // @ts-ignore
      const result = await (this.constructor as typeof ORMAdapterSQLite).getRow(this.database, sql, [...values]);
      return result ? [result] : [];
    }

    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).getRows(this.database, sql, [...values]);
  }

  async readAll(kv: Map<string, any> | null, columns=['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
    // @ts-ignore
    const statementOrderBy = (this.constructor as typeof ORMAdapterSQLite).getOrderByStatement(orderBy);
  ...
  async readWith(criteria: any[][], columns=['id', 'name'], limit = 1000, offset = 0, orderBy = new Map([['id', 'ASC']])) {
    ...
    const statementOrderBy = (this.constructor as typeof ORMAdapterSQLite).getOrderByStatement(orderBy);
    // @ts-ignore
    const { wheres, whereValues } = (this.constructor as typeof ORMAdapterSQLite).getWheresAndWhereValueFromCriteria(criteria);
  ...
  async countAll(kv: Map<string, any> | null = null) {
    ...
    // @ts-ignore
    const result = await (this.constructor as typeof ORMAdapterSQLite).getRow(this.database, `SELECT COUNT(id) FROM ${this.tableName}${where}`, v);
    return result['COUNT(id)'];
  }

  async countBy(key: string, values: any[]) {
    // @ts-ignore
    const result = await (this.constructor as typeof ORMAdapterSQLite).getRow(this.database,
  ...
  async deleteAll(kv: Map<string, any> | null = null) {
    // @ts-ignore
    if (!kv) return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${this.tableName}`, []);
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${this.tableName} WHERE ${Array.from(kv.keys()).map(k => k + ' = ?').join(' AND ')}`, Array.from(kv.values()))
  }

  async deleteBy(key: string, values: any[]) {
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${this.tableName} WHERE ${key} IN (${values.map(() => '?').join(', ')})`, values);
  }
  ...
  async deleteWith(criteria: any[][]) {
    ...
    const { wheres, whereValues } = (this.constructor as typeof ORMAdapterSQLite).getWheresAndWhereValueFromCriteria(criteria);
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `DELETE FROM ${this.tableName} WHERE ${wheres}`, whereValues);
  }
  ...
  async updateAll(kv: Map<string, any> | null, columnValues: Map<string, any>) {
    ...
    // @ts-ignore
    if(!kv) return (this.constructor as typeof ORMAdapterSQLite).run(this.database, `UPDATE ${this.tableName} SET ${keys.map(key => key + ' = ?')}`, newValues);

    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run( this.database,
  ...
  async insertAll(columns: string[], valueGroups: any[][], ids: any[]=[]) {
    ...
    // @ts-ignore
    return (this.constructor as typeof ORMAdapterSQLite).run(
      this.database,
